...
self.actuation_polytope_total_3d = geometry_computation.minkowskiSum(self.leg_actuation_wrench_polytope_total_3d, self.ascender_actuation_force_polytope);
...
% Polytope를 정의하는 불평등 제약식 만들기
Kqp = convhulln(Vqp(1:2,:)'); % 2D convex hull 사용 (첫 두 축만 사용)
Aqp = [];
bqp = [];
...
% Objective function: 거리의 제곱 최소화 (||x - robot_pos||^2)
Hqp = eye(2); % 2D 문제이므로 2x2 단위 행렬
fqp = -robot_pos(1:2); % 목표 위치에서 로봇의 위치

try
    % quadprog를 이용한 최적화 문제 해결 (Ax <= b)
    x_qp = quadprog(Hqp, fqp, Aqp, bqp);
    x_qp_full = [x_qp; 0]; % z값 0 추가
catch
    % 최적해가 없을 경우 (예외 처리)
    x_qp_full = self.p_base;
end

self.base2move = x_qp_full - self.p_base;

% 결과 출력
disp('Polytope 내부에서 가장 가까운 점:');
disp(x_qp_full);

disp('base movement:');
disp(x_qp_full - self.p_base);
...
plotting_tools.plot_fesible_polytopes2(self);
