function Psum = minkowskiSum_3D(PA,PB)
% computes the minkowski sum of two general polygons
% usage: Psum = minkowskiSum(PA,PB)
%
% arguments: (input)
% PA - a 3-d polyshape, or an nx2 array of x,y coordinates traversing a polygon
% PB - a 3-d polyshape, or an mx2 array of x,y coordinates traversing a polygon
%
% arguments: (output)
% Psum - a 3-d polyshape that represents the resulting Minkowski sum.
%
% Example of use:
...
if ~isa(PA,'convHull')
  PA_convhull = convhull(PA);
end

if ~isa(PB,'convHull')
  PB_convhull = convhull(PB);
end

% triangulate each polyshape
Atri = triangulation(PA_convhull,PA);
Btri = triangulation(PB_convhull,PB);
...
for iA = 1:NA
  for iB = 1:NB
    ...
    HexShape = convhull(Hxy(Hexhull,:));
    
    if (iA == 1) && (iB == 1)
      Psum = HexShape;
    else
      Psum = convhull(Psum,HexShape);
    end
  end
end
